...
#        self.dn = 0    #当前结点的深度
    ...
    def GetWn(self): #更新当前结点不在位的将牌数
        count = 0
        for x in range(0,3):   #计算在位的将牌数
            tempState = self.state.copy()
            tempAnswer = self.answer.copy()
            for y in range(0,3):
                if(tempState[x,y] == tempAnswer[x,y]):
                    count += 1
        self.wn = 8 - count
